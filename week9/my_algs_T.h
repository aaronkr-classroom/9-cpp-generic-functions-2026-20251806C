...
bool not_equals(const T& left, const T& right) {
	//return left == right ? flase : true;
	//return left != right ? true : flase;
...
Out copy(In begin, In out, Out dest) {
	//첫 요소부터 끝까지 모든
	//begin 반복자의 있는 요소를
	//dest 반복자로 복사하기
	while (begin != end) {
...
//이진법 탐색
template<class Out,class X>
bool binary_search(Out begin,Out end,const X&x){
	while (begin < end)[
		//범위의 중간 지점 찾기
		Out mid = begin + (end - begin) / 2;
...
void split(const string& str, Out os) {
	...
	while (i != str.end()) {
		//선행 공백을 무시
		//지금 i위치로부터 첫 번쨰 뛰어쓰기 아닌 문자 찾기
		i = find_if(i, str.end(), not_space);

		//다음 단어의 끝을 찾음
		//while 반복할 때 항상 새로운 반복자 j를 생성
		//지금 j위치로부터 첫 번쨰 뛰어쓰기 문자 찾기
